...
func TestInsertOpentsdbTelnet(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if _, err := capi.InsertOpentsdbTelnet(tt.args.taosConnect, []string{tt.args.data}, tt.args.db, tt.args.ttl, 0); (err != nil) != tt.wantErr {
...
func BenchmarkTelnet(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		//t_98df8453856519710bfc2f1b5f8202cf
		_, err := capi.InsertOpentsdbTelnet(conn, []string{`put sys.if.bytes.out 1479496100 1.3E3 host=web01 interface=eth0`}, "test", 0, 0)
		if err != nil {
			b.Error(err)
		}
	}
				...
				data: []byte(`
{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if _, err := capi.InsertOpentsdbJson(tt.args.taosConnect, tt.args.data, tt.args.db, tt.args.ttl, 0); (err != nil) != tt.wantErr {
...
func TestInsertOpentsdbTelnetBatch(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if _, err := capi.InsertOpentsdbTelnet(tt.args.taosConnect, tt.args.data, tt.args.db, tt.args.ttl, 0); (err != nil) != tt.wantErr {
				t.Errorf("InsertOpentsdbTelnet() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
